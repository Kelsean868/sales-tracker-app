...
const PortfolioScreen = ({ clients = [], policies = [], onSelectClient, onSelectPolicy, fetchMoreData, loadingMore }) => {
    ...
    const lastElementRef = useCallback(node => {
        ...
        observer.current = new IntersectionObserver(entries => {
            if (entries[0].isIntersecting) {
                if (filter.itemType === 'All' || filter.itemType === 'Client') {
                    fetchMoreData('clients');
                }
                if (filter.itemType === 'All' || filter.itemType === 'Policy') {
                    fetchMoreData('policies');
                }
            }
        });
        if (node) observer.current.observe(node);
    }, [loadingMore, fetchMoreData, filter.itemType]);
    ...
    const filteredAndSortedItems = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            items = items.filter(item => {
                if (item.itemType === 'Client') return item.name.toLowerCase().includes(lowercasedSearchTerm);
                if (item.itemType === 'Policy') {
                    ...
                    return item.policyNumber.toLowerCase().includes(lowercasedSearchTerm) || ownerName.includes(lowercasedSearchTerm) || insuredName.includes(lowercasedSearchTerm) || payorName.includes(lowercasedSearchTerm) || beneficiaryName.includes(lowercasedSearchTerm);
                }
                return false;
            });
        }
    ...
    return (
        <div className="space-y-4">
            ...
            <FilterSortPanel filterOptions={filterOptions} sortOptions={sortOptions} filter={filter} setFilter={setFilter} sort={sort} setSort={setSort} />
            <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
                <input type="text" placeholder="Search portfolio..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-gray-800 border-gray-700 rounded-md p-2 pl-10 focus:ring-amber-500 focus:border-amber-500" />
            </div>
            <div className="space-y-3">
                ...
                {loadingMore && <p className="text-center py-4">Loading more...</p>}
            </div>
        </div>
    );
};
...
const ClientCard = ({ client, onClick }) => (
    <Card onClick={onClick} className="cursor-pointer hover:border-amber-500 transition-colors">
        <div className="flex justify-between items-start">
            ...
            <div className="text-xs bg-blue-500/20 text-blue-300 px-2 py-1 rounded-full">Client</div>
        </div>
...
const PolicyCard = ({ policy, onClick }) => (
     <Card onClick={onClick} className="cursor-pointer hover:border-teal-500 transition-colors">
        <div className="flex justify-between items-start">
            ...
            <div className="text-xs bg-green-500/20 text-green-300 px-2 py-1 rounded-full">{policy.status}</div>
        </div>
        <div className="mt-3 text-sm text-gray-300 space-y-1 pl-7">
