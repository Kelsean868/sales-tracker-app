...
const LeadsScreen = ({ leads, fetchMoreData, loadingMore, onSelectLead, allUsers = [], currentUser, onUpdateLead }) => {
    ...
    const handleDrop = (e, newStage) => {
        ...
        if (leadToUpdate) onUpdateLead({ ...leadToUpdate, stage: newStage });
    };
...
const PipelineView = ({ leads, onSelectLead, isManagerView, getUserName, onDragStart, onDrop, onDragOver, lastElementRef }) => (
    <div className="flex space-x-4 overflow-x-auto pb-4" ref={lastElementRef}>
        {Object.values(LEAD_STAGES).map(stage => (
            <div key={stage} className="w-72 bg-gray-800 rounded-lg p-3 flex-shrink-0" onDrop={(e) => onDrop(e, stage)} onDragOver={onDragOver}>
                ...
                <div className="space-y-3">
                    {leads.filter(lead => (lead.stage || LEAD_STAGES.NEW) === stage).map(lead => (
                        <LeadCard key={lead.id} lead={lead} onClick={() => onSelectLead(lead)} userName={isManagerView ? getUserName(lead.userId) : null} draggable onDragStart={(e) => onDragStart(e, lead.id)} />
                    ))}
                </div>
            </div>
        ))}
...
const LeadCard = ({ lead, onClick, userName, ...props }) => (
    <Card onClick={onClick} className="cursor-pointer hover:border-amber-500 transition-colors" {...props}>
